...
{
  fprintf(stdout, "table_len %zu, load %zu, count %zu, new_table_len %zu\n",
...
int dict_entry_set(struct dict *dict, struct dict_entry *ent, hash_t hash, char *key)
{
  ...
  if (3 * new_load >= 2 * dict->d_table_len) {
    ...
    if (new_count >= new_table_len) {
      fprintf(stdout, "new_count %zu >= new_table_len %zu\n", new_count, new_table_len);
      errno = ENOMEM;
      return -1;
    }
...
int dict_set(struct dict *dict, char *key)
{
  ...
  if (ent->d_key != NULL) {
    fprintf(stdout, "overwriting old key `%s', hash %zu, with new key `%s' hash %zu\n",
...
int dict_add(struct dict *d, char *key, char *val) {
  int rc = -1;
  char *key_val = malloc(strlen(key) + 1 + strlen(val) + 1);
  strcpy(key_val, key);
  strcpy(key_val + strlen(key) + 1, val);
  if (dict_set(d, key_val) < 0) {
    free(key_val);
    return rc;
  }
  return 0;
...
char *dict_get(struct dict *d, char *key) {
  return dict_ref(d, key) + strlen(key) + 1;
}
